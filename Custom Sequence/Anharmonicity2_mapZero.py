...
class CustomSequence(Sequence):
    def generate_sequence(self, config):
         ...
         remap=config.get('Parameter #9') #add Pi pulse on 01 if the value 1
         ...
         # self.add_gate_to_all(Gate.Xp)
         PiPulseGE=Pulse()
         PiPulseGE.width = 75e-9
         PiPulseGE.plateau = 0
         PiPulseGE.amplitude = 259.5e-3*remap
         PiPulseGE.frequency = -50e6
         PiPulseGE.shape=PulseShape.COSINE
         gatePi=CustomGate(PiPulseGE)
         self.add_gate_to_all(gatePi)
